...
(function($) {
  ...
  loadTracksData = function($player, links, key) {
    ...
    loadUrl = function(link) {
      ...
      request.error(function(x, t, m) {
        if(t === "timeout") {
          console.log('track timed out.. maybe it doesn\'t exist or is private?');
        }
        index += 1;
      ...
      // load first tracks
      // this makes an extra, potentially unnecessary api call
      loadUrl(links[index]);
    },
        ...
        $('.sc-artwork-list li', $player).each(function(index) {
          ...
          if (itemTrack === track) {
            ...
            .each(function(index) {
                  ...
                  $(this).html(artworkImage(track, false));
                });
  ...
  $(document).on('click', 'a.sc-previous', function(event) {
    ...
    scrollToPrevTrack(track, last_track);
    track.click();
  ...
  $(document).on('click', 'a.sc-next', function(event) {
    ...
    scrollToNextTrack(track, first_track);
    track.click();
  ...
  $(document).on('click','.sc-trackslist li:not(.divider)', function(event, data) {

    var $track = $(this),
    $player = $track.closest('.sc-player'),
    trackId = $track.data('sc-track').id,
    trackNotPlaying = $player.is(':not(.playing)') || $track.is(':not(.active)');
  ...
  $.scPlayer.loadTrackInfoAndPlay = function($elem, track, meta, initialLoad) {
    var playerObj = players[$elem.data('sc-player').id];
          ...
          if(!initialLoad) { 
           $li.click();
           $('.sc-trackslist').animate({ scrollTop: 0 });
         }

       }

       $.scPlayer.loadTrackUrlAndPlay = function($elem, track, initialLoad) {
        ...
        $.getJSON(apiUrl, function(data) {
          if(data.duration){
            ...
            $.scPlayer.loadTrackInfoAndPlay($elem, data, track, initialLoad);
          }
        });
      }
...
jQuery(document).bind('onPlayerTrackSwitch.scPlayer', function(event, track) {
  ...
  jQuery('#player').data('waveform_url', track.waveform_url);
    ...
    if(jQuery('#player').hasClass('loaded') && window.current_track != track.permalink) {

      jQuery('#large-waveform canvas').remove();
      initialize_waveform(track);
    } 

    window.current_track = track.permalink;

    updatePlayerHeight();

  });

function updatePlayerHeight(prepend_track) { 
  ...
  function prependTrackToFront() {
    ...
    if(jQuery('.sc-trackslist').find('li.active').index() > 0) {
      ...
      jQuery('.sc-trackslist').css('overflow-y', 'scroll').animate({ scrollTop: trackslist.position().top }); 

    }
  }


  function scrollToNextTrack(track, first_track) {
    var trackslist = jQuery('.sc-trackslist');
    
    if(track.position().top + (track.height() / 2) > trackslist.height()) {
      trackslist.animate({ scrollTop: track.position().top }); 
    } else if(first_track) {
      trackslist.animate({ scrollTop: 0 });
    }
  }

  function scrollToPrevTrack(track, last_track) {
    var trackslist = jQuery('.sc-trackslist');

    if(track.position().top < 0 || last_track) {
      trackslist.animate({ scrollTop: track.position().top }); 
    }
  }
...
function big_waveform(track_position, mouse_position) { 
  ...
  if(jQuery('#large-waveform').length && window.sc_data != null) {
